...
static void	render_background(t_image *canvas,
			const t_colour ceiling, const t_colour floor)
{
	const unsigned int	len = canvas->size.y * canvas->size.x;
	const unsigned int	half = len / 2;

	ft_intset((int *)canvas->data, half, ceiling);
	ft_intset((int *)canvas->data + half, half + (len & 0b1), floor);
}

typedef void	(*t_ftrender)(t_image *canvas, const t_rays rays,
			...
			const t_image *images, const char *target)
{
	...
	ft_render(&game->canvas, game->rays, images, &game->map);
	image_put(game->mlx, &game->canvas, (t_point){0, 0});
}

void	render(t_game *game)
{
	render_background(&game->canvas,
		game->texture.colour_ceiling,
		game->texture.colour_floor
	);
